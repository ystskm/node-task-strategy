...
var SDB = require('./strategies-db'), ODB = require('./oplog-db');
...
function Strategy(db, tactics, configs, options) {
  ...
  this._index = -1, this._tfns = {}, this._sfns = {};

  this._sdb = new SDB(db), this._sdb.on('ready', function() {
...
function next(signalOk) {
  ...
  function nextTactics() {
    ...
    if(typeof tfn != 'function')
      return self.emit('error', new Error('Not a function tactics: '
        + tacticsName + ' (' + typeof tfn + ')'));
...
function fatal(e) {
  this.__stopby = 'fatal';
  this.emit('error', typeof e == 'string' ? new Error(e): e);
  this._sdb.update({
    state: State.Fatal,
    errmsg: e.message,
    estack: e.stack

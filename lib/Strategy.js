...
Object.keys(StraProtos = {
  ...
  Strategy.prototype[k] = _wrap(StraProtos[k]);
});

Tactics.StrategyEvent = Strategy.Event = Event;
Tactics.StrategyState = Strategy.State = State;
...
function _wrap(fn) {
  ...
  return function() {
    try {
      ...
      st.__stopby == Event.Error || st.emit(st.__stopby = Event.Error, e);
      throw e;
...
function next(signalOk) {
  ...
  function nextTactics() {
    ...
    // Go to next index! (pos = +1)
    // console.log('[Strategy] tactics:', st._tactics, st._index);
    var tacticsName = st._tactics[pos = (st._index += 1)];
    if(tacticsName == NULL) {
      return st.emit(Event.End);
    }
